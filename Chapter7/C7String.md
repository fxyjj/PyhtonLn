...
(还有另外一个用于查找子字符串偏移量的方法str.index(),这个方法是序列类型通用的，他的作用与find类似，不过如果找不到子串的话会报错)
```
...
1. string.split(sep[,maxsplit]), 将字符串转换成一个数组，其中的元素为通过指定分割副分割成的字符串的不同部分. *注意如果字符串中没有合适的分割符，就使用list()内置函数来将字符串分解成单个字符组成的数组*
...
Python的字符串格式化可以以两种方式实现，格式化表达式和格式化方法。
1. 格式化表达式： Python诞生之初就存在的技术，类似于C语言的'printf'模型
2. 格式化方法调用： Python2.6 & python3.0 中新增的技术，这一形式部分起源于C#/.NET 中的同名工具。
**由于字符串不可变，所以格式化操作同样会创建新的字符串而不是在原来的字符串中修改**

### 4.1 字符串格式化表达式

* 使用 `%` 二进制运算符，该运算符提供了根据格式定义，将不同类型的值格式化成字符串的简单方法。
* 具体步骤：
    1. 在`%`左侧放置一个需要格式化的字符串，该字符串带有一个或多个内嵌的转换目标，都以`%`开头.
    2. 在`%`右侧放置一个(或多个内嵌在元组中的)对象，这些对象将会插入到你想让Python进行格式化的左侧字符串中，并替换一个或多个转换目标。
...
>>> 'The Knight who says %d\n'%damn
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
TypeError: %d format: a number is required, not str

>>> 'The Knight who says %s\n'%damn
'The Knight who says Hello\n'
>>> 
```
**格式化类型需要匹配，%d转换的是数字类型的子串**
     2. %s格式化可转换成字符串的类型
```
>>> '%s---%s---%s'%(42,3.1415,[1,2,3])
'42---3.1415---[1, 2, 3]'
```
**由于整数，浮点数和数组都可以转换成字符串（用print打印出来），所以他们都可以使用%s转换**

* 高级格式化表达式语法：表达式左侧的转换目标支持多种转换操作，其标准格式如：

```%[(keyname)][flags][width][.precision]typecode```

1. keyname: 为引索在表达式右侧使用的字典提供建名称
2. flags： 说明格式的标签， (+)数值符号,(-)左对齐，(空格)正数前的空白和负数前的-，(0)零填充
